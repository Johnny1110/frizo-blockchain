...
// NewBatch 創建批量操作
func (db *MemoryDatabase) NewBatch() db.Batch {
	return &memBatch{
		db:     db,
...
// NewIterator 創建迭代器
func (db *MemoryDatabase) NewIterator(prefix []byte, start []byte) db.Iterator {
	db.mu.RLock()
	defer db.mu.RUnlock()
...
func (b *memBatch) Replay(target db.Batch) error {
	b.mu.Lock()
	defer b.mu.Unlock()
